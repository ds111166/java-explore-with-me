...
public class PublicEventController {
    ...
    public List<EventGetPublicResponse> getPublicEvents(
            @RequestParam(required = false) String text, //текст для поиска в аннотации и подробном описании события
            ...
            @RequestParam(required = false, defaultValue = "false") Boolean onlyAvailable, //только события у которых
            // не исчерпан лимит запросов на участие
            @RequestParam(required = false) String sort, //по дате события или по кол просмотров (EVENT_DATE, VIEWS)
        ...
        List<EventGetPublicResponse> publicEvents = eventService.getPublicEvents(request, text,
    ...
    @GetMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    public EventResponseDto getPublicEventById(@PathVariable @NotNull Long id, HttpServletRequest request) {
        log.info("Получение публичного события по id={}", id);
        EventResponseDto publicEvent = eventService.getPublicEventById(request, id);
